...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        im_size = (28, 28)
        num_classes = 10
        mean = [0.0711,0.0680,0.0642]
        std = [0.2040,0.1907,0.1788]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        save_data = torch.load(os.path.join('/root/autodl-tmp/Biased-DD-main/dataset/CMNIST-DD/',data_path))
        ...
        print(dst_train_img.shape,dst_train_lab.shape,dst_train_attr.shape)
        for c in range(channel):
            dst_train_img[:,c] = (dst_train_img[:,c] - mean[c])/std[c]
            dst_test_img[:,c] = (dst_test_img[:,c] - mean[c])/std[c]
        dst_train=[dst_train_img, dst_train_lab, dst_train_attr]
        dst_test=[dst_test_img, dst_test_lab, dst_test_attr]
        ...
        class_names = ["climbing", "diving", "fishing", "pole vaulting", "racing", "throwing"]

        mean = [0.4078,0.4742,0.4873]
        std = [0.2742,0.2428,0.2730]
        
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std),
                                        transforms.Resize(im_size),
                                        transforms.CenterCrop(im_size)])
        dst_train = datasets.ImageFolder(os.path.join("/root/autodl-tmp/BiasedDatasets/BAR-master/", "train"), transform=transform) # no augmentation
        dst_test = datasets.ImageFolder(os.path.join("/root/autodl-tmp/BiasedDatasets/BAR-master/", "test"), transform=transform)
    elif dataset == 'wb':
        channel = 3
        im_size = (128,128)
        num_classes = 2
        ...
        mean = [0.4118, 0.4256, 0.3500]
        std = [0.2451, 0.2412, 0.2590]
        
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std),
                                        transforms.Resize(im_size),
                                        transforms.CenterCrop(im_size)])
        dst_train = datasets.ImageFolder(os.path.join("/root/autodl-tmp/BiasedDatasets/waterbird_complete95_forest2water2/", "train"), transform=transform) # no augmentation
        dst_test = datasets.ImageFolder(os.path.join("/root/autodl-tmp/BiasedDatasets/waterbird_complete95_forest2water2/", "test"), transform=transform)
    elif dataset == 'bffhq':
        channel = 3
        im_size = (128,128)
        num_classes = 2
        ...
        mean = [0.5274, 0.4325, 0.3888]
        std = [0.2795, 0.2541, 0.2554]
        
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std),
                                        transforms.Resize(im_size),
                                        transforms.CenterCrop(im_size)])
        dst_train = datasets.ImageFolder(os.path.join("/root/autodl-tmp/BiasedDatasets/bffhq/", "train"), transform=transform) # no augmentation
        dst_test = datasets.ImageFolder(os.path.join("/root/autodl-tmp/BiasedDatasets/bffhq/", "test"), transform=transform)
    elif dataset == 'CCIFAR10':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.5345,0.5283,0.4907]
        std = [0.2445,0.2393,0.2640]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        save_data = torch.load(os.path.join('/root/autodl-tmp/Biased-DD-main/dataset/CCIFAR10-DD/',data_path))
        dst_train_img, dst_train_lab, dst_train_attr = save_data["training_imgs"], save_data["training_labs"], save_data["training_attrs"]
        dst_test_img, dst_test_lab, dst_test_attr = save_data["testing_imgs"], save_data["testing_labs"], save_data["testing_attrs"]
        print(dst_train_img.shape,dst_train_lab.shape,dst_train_attr.shape)
        for c in range(channel):
        ...
        # dst_test = datasets.FashionMNIST(data_path, train=False, download=True, transform=transform)
        class_names = 10
    elif dataset == 'CCIFAR10V2':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4893,0.4797,0.4443]
        std = [0.2458,0.2426,0.2602]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        save_data = torch.load(os.path.join('/root/autodl-tmp/Biased-DD-main/dataset/CCIFAR10V2-DD/',data_path))
        dst_train_img, dst_train_lab, dst_train_attr = save_data["training_imgs"], save_data["training_labs"], save_data["training_attrs"]
        dst_test_img, dst_test_lab, dst_test_attr = save_data["testing_imgs"], save_data["testing_labs"], save_data["testing_attrs"]
        print(dst_train_img.shape,dst_train_lab.shape,dst_train_attr.shape)
        for c in range(channel):
...
def get_loops(ipc):
    ...
    if ipc == 1:
        ...
        outer_loop, inner_loop = 70, 5
    elif ipc == 100:
        outer_loop, inner_loop = 100, 5
    elif ipc >= 100:
        outer_loop, inner_loop = ipc, 3
    else:
...
def evaluate_synset_bar(it_eval, net, images_train, labels_train,testloader, args):
    net = net.to(args.device)
    ...
    for ep in range(Epoch+1):
        ...
        if ep in lr_schedule:
            ...
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        # if ep%20==0:
        #     print('%s Evaluate_%02d: epoch = %04d train loss = %.2f train acc = %.2f' % (get_time(), it_eval, ep, loss_train, acc_train))
...
def evaluate_synset_bar_mix(it_eval, net, images_train, labels_train, image_syn, label_syn, testloader, args):
    ...
    start = time.time()
    for ep in range(Epoch+1):
        loss_train, acc_train = epoch_mix('train', trainloader, image_syn, label_syn, net, optimizer, criterion, args, aug = True)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        # if ep%20==0:
...
def epoch_mix(mode, dataloader, image_syn, label_syn, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    criterion = criterion.to(args.device)
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        base_indices = lab * 50
        rand_indices = torch.randint(0, 50, (len(lab),)).cuda()
        final_indices = base_indices + rand_indices
        selected_images = image_syn[final_indices]
        interpolated_images = img * 0.8 + selected_images * 0.2
        img = torch.cat((img, interpolated_images), dim=0)
...
def evaluate_synset(it_eval, net, images_train, labels_train, images_test_all,labels_test_all,testloader, args):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    lr = float(args.lr_net)
    ...
    dst_test = TensorDataset(images_test_all, labels_test_all)
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=256, shuffle=False, num_workers=0)
...
def evaluate_synset_mix(it_eval, net, images_train, labels_train, image_syn, label_syn, images_test_all,labels_test_all,testloader, args):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M': # multiple architectures
        ...
        model_eval_pool = ['ConvNet', 'AlexNetBN', 'ResNet18BN'] ###
